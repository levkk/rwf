...
use openidconnect::{AccessTokenHash, AuthorizationCode, Client, CsrfToken, EmptyAdditionalClaims, Nonce, OAuth2TokenResponse, PkceCodeChallenge, PkceCodeVerifier, StandardErrorResponse, TokenResponse, UserInfoClaims, reqwest, AccessToken, RefreshToken, StandardTokenResponse};
...
impl VerifyData {
    fn new(verifier: PkceCodeVerifier, _csrf: CsrfToken, nonce: Nonce, target: crate::http::path::Path) -> Self {Self {verifier, _csrf, nonce, target}}
}
...
async fn clients(config:  &crate::config::OidcConfig) -> Result<(reqwest::Client, RWFOidcClient), Error> {
    if !config.everything_set() {
        return Err(Error::Config(crate::config::Error::NoConfig))
    }
    let http_client = reqwest::ClientBuilder::new().redirect(reqwest::redirect::Policy::none()).build().map_err(|e| Error::Error(Box::new(e)))?;
    debug!("{:?}", http_client);
    let metadata = CoreProviderMetadata::discover_async(config.discovery_url.as_ref().unwrap().clone(), &http_client).await.map_err(|e| Error::Error(Box::new(e)))?;
    debug!("{:?}", metadata);
    let client: RWFOidcClient = CoreClient::from_provider_metadata(metadata, config.client_id.as_ref().unwrap().clone(), config.client_secret.clone()).set_redirect_uri(config.redirect_url.as_ref().unwrap().clone());
...
pub trait OidcUser: Model {
    ...
    /// }
    /// ```
    async fn from_token(token: StandardTokenResponse<CoreIdTokenFields, CoreTokenType>, userinfo: UserInfoClaims<EmptyAdditionalClaims, CoreGenderClaim>) -> Result<Self, crate::model::Error>;
...
impl<U: OidcUser> Modify for OidcAuthentication<U> {
    fn modify(&self, openapi: &mut utoipa::openapi::OpenApi) {
        ...
        if !config.oidc.everything_set() {return}
        ...
        let requirement = utoipa::openapi::SecurityRequirement::new("oidc_authentication", scopes.clone()).add("oauth2_authcode", scopes);
        ...
        for path in openapi.paths.paths.values_mut() {
            for mut operation in [path.get.as_mut(), path.post.as_mut(), path.put.as_mut(), path.patch.as_mut(), path.head.as_mut()] {
...
impl<U: OidcUser + Send + Sync> Authentication for OidcAuthentication<U> {
    async fn authorize(&self, request: &Request) -> Result<bool, Error> {
        ...
        if !user.is_expired() {
            ...
            let res = oidc_client.exchange_refresh_token(&user.refresh_token()).map_err(|e| Error::Error(Box::new(e)))?.request_async(&http_client).await.map_err(|e| Error::Error(Box::new(e)))?;
    ...
    async fn denied(&self, request: &Request) -> Result<Response, Error> {
        let config = get_config();
        let (http_client, client) = clients(&config.oidc).await?;
        ...
        debug!("{:#?}", VERIFY_MAP);
        VERIFY_MAP.map.write().await.insert(csrf_token.secret().clone(), VerifyData::new(pkce_verifier, csrf_token, noonc, request.path().clone()));
    ...
    async fn handle(&self, request: &Request) -> Result<Response, Error> {
        ...
        if let Some(state) = request.query().get::<String>("state") {
            ...
            if let Some((_, verify)) = VERIFY_MAP.map.write().await.remove_entry(state.as_str())  {
                let auth_code = AuthorizationCode::new(request.query().get_required::<String>("code")?);
                let token = oidc_client.exchange_code(auth_code).map_err(|e| Error::Error(Box::new(e)))?.set_pkce_verifier(verify.verifier).request_async(&http_client).await.map_err(|e| Error::Error(Box::new(e)))?;
                let id_token = token.id_token().ok_or(crate::http::Error::MissingParameter)?;
                let id_token_verifier = oidc_client.id_token_verifier();
                let claim = id_token.claims(&id_token_verifier, &verify.nonce).map_err(|e| Error::Error(Box::new(e)))?;
                if let Some(expected_token_hash) = claim.access_token_hash() {
                    ...
                    let userinfo: UserInfoClaims<EmptyAdditionalClaims, CoreGenderClaim> = oidc_client
                    ...
                    Err(Error::HttpError(Box::new(crate::http::Error::MalformedRequest("Invalid Clain Hasth"))))
                }
            } else {
                Err(Error::SessionMissingError)
            }
        } else {
            Err(Error::HttpError(Box::new(crate::http::Error::MissingParameter)))
        }
    }
}